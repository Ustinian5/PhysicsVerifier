...
# TRAIN_STAGE=bootstrap (default): 3 train GPUs + 1 local 30B judge, process_paragraph.
# TRAIN_STAGE=verifier: 3 train GPUs + 1 local judge GPU, answer_low_verifier.
set -euo pipefail
...
if [[ "${TRAIN_STAGE}" == "bootstrap" ]]; then
  ...
  export PHYSICS_REWARD_PARA_MIN="${PHYSICS_REWARD_PARA_MIN:-220}"
  export PHYSICS_REWARD_PARA_TARGET="${PHYSICS_REWARD_PARA_TARGET:-320}"
  export PHYSICS_REWARD_PARA_MAX="${PHYSICS_REWARD_PARA_MAX:-400}"
...
if [[ "${PHYSICS_REWARD_MODE}" != "answer_only" ]]; then
  ...
  env -u PHYSICSVERIFIER_OPENAI_BASE_URL -u PHYSICSVERIFIER_OPENAI_API_KEY \
    VENV="${TRAIN_VENV:-/data1/jinjianhan/venv/openrlhf_train}" \
    ...
    PHYSICS_REWARD_CONCURRENCY="${PHYSICS_REWARD_CONCURRENCY:-12}" \
    PHYSICS_REWARD_MAX_RESPONSE_CHARS="${PHYSICS_REWARD_MAX_RESPONSE_CHARS:-3072}" \
    PHYSICSVERIFIER_UNIFIED_RULE_TOP_N="${PHYSICSVERIFIER_UNIFIED_RULE_TOP_N:-4}" \
    PHYSICSVERIFIER_PRECISION_MODE="${PHYSICSVERIFIER_PRECISION_MODE:-balanced}" \
    PHYSICSVERIFIER_UNIFIED_RETRIEVAL_MODE="${PHYSICSVERIFIER_UNIFIED_RETRIEVAL_MODE:-lexical}" \
